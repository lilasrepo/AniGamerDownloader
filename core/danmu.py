#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Author  : Miyouzi (original Danmu.py); refactored into core by local fork
"""Danmu (彈幕) downloader, refactored for dependency injection.

Moved from ``src/Danmu.py``. Two seams inverted vs the original:

* The ``DanmuTemplate.ass`` path is now INJECTED via the ``danmu_template``
  keyword arg (was hard-coded as ``Config.get_working_dir()/data/DanmuTemplate.ass``
  at the old line 70).
* ``err_print`` is INJECTED via ``logger`` (default = the module-level
  ``core.logging.err_print``), so there is no ``import Config`` /
  ``from ColorPrint import err_print`` module coupling.

Cookies were already passed via the constructor in the original; that is
unchanged. The download() body is byte-for-byte identical apart from the two
injected seams.
"""
...
class Danmu():
    def __init__(self, sn, full_filename, cookies, *, danmu_template, logger=None):
        self._sn = sn
        self._full_filename = full_filename
        self._cookies = cookies
        self._danmu_template = danmu_template
    ...
    def find_ban_word(self, text, ban_word_re):
        ...
        # confirm it did not match an empty string
    ...
    def download(self, ban_words):
        ...
        if r.status_code != 200:
            self._log(self._sn, '彈幕下載失敗', 'status_code=' + str(r.status_code), status=1)
            return
        ...
        for danmu in j:
            ...
            if self.find_ban_word(text, ban_word_re):
                self._log(self._sn, f'跳過彈幕 [{text}]', self._full_filename, display=False)
                continue
        ...
        self._log(self._sn, '彈幕下載完成', self._full_filename, status=2)
